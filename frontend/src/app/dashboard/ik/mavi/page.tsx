...
export default function Page(){
    ...
    const exportTripsCsv = async ()=>{
        await saveCsv('mavi_yol_yaklasan_seferler.csv',
            ['ID','Sefer','Kalkış','Varış','Tarih','Satış','Gelir','Aktif'],
            upcoming.map(t=>[
                t.id, t.trip||'-', t.terminal_from||'-', t.terminal_to||'-',
                fmt(t.departure_time), Number(t.orders||0), String(t.revenue ?? 0),
    ...
    const exportCustomersCsv = async ()=>{
        const list = custs.slice(0,8);
        await saveCsv('mavi_yol_en_iyi_musteriler.csv',
    ...
    const exportApprovalsCsv = async ()=>{
        await saveCsv('mavi_yol_bekleyen_onaylar.csv',
            ['ID','Ad','E-posta','Firma Onayı','Admin Onayı','Başvuru'],
            pendingApprs.map(r=>[
                r.user?.id ?? '', r.user?.name||'-', r.user?.email||'-',
                r.company_status, r.admin_status, fmt(r.created_at)
            ])
        );
    };
    // --- CSV end ---
    ...
    return (
        <div className="space-y-6 text-indigo-900">
            <div className="flex items-center justify-between">
                <h1 className="text-2xl font-bold">Mavi Yol • Genel Bakış</h1>
                <button
            ...
            <div className="grid gap-4 lg:grid-cols-2">
                ...
                <section className="rounded-2xl border bg-white p-4">
                    <div className="mb-2 flex items-center justify-between">
                        ...
                        <button onClick={exportTripsCsv} className="rounded-lg border px-2 py-1 text-sm">CSV</button>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="min-w-[900px] w-full text-sm">
                            ...
                            {!upcoming.length && <tr><td colSpan={8} className="py-6 text-center text-indigo-900/50">Kayıt yok</td></tr>}
                            </tbody>
                        </table>
                    </div>
                </section>
                ...
                <section className="rounded-2xl border bg-white p-4">
                    <div className="mb-2 flex items-center justify-between">
                        ...
                        <button onClick={exportCustomersCsv} className="rounded-lg border px-2 py-1 text-sm">CSV</button>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="min-w-[800px] w-full text-sm">
                            ...
                            {!custs.length && <tr><td colSpan={5} className="py-6 text-center text-indigo-900/50">Kayıt yok</td></tr>}
                            </tbody>
                        </table>
                    </div>
                </section>
                ...
                <section className="rounded-2xl border bg-white p-4 lg:col-span-2">
                    <div className="mb-2 flex items-center justify-between">
                        ...
                        <button onClick={exportApprovalsCsv} className="rounded-lg border px-2 py-1 text-sm">CSV</button>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="min-w-[900px] w-full text-sm">
                            <thead>
                            ...
                            {!pendingApprs.length && <tr><td colSpan={6} className="py-6 text-center text-indigo-900/50">Bekleyen onay yok</td></tr>}
                            </tbody>
                        </table>
                    </div>
                </section>
...
function KPI({title, value, sub}:{title:string; value:string; sub?:string}){
    return (
        <div className="rounded-2xl border bg-white p-4">
            ...
            <div className="mt-1 text-2xl font-bold">{value}</div> {sub ? <div className="text-xs text-indigo-900/50 mt-0.5">{sub}</div> : null} </div> ); }