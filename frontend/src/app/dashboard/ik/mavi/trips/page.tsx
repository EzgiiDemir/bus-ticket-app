// app/dashboard/Company/trips/page.tsx
'use client';
...
export default function CompanyTrips(){
    ...
    const [rows,setRows]=useState<Trip[]>([]); const [err,setErr]=useState(''); const [q,setQ]=useState('');
    useEffect(()=>{ (async ()=>{
        try{
    ...
    const exportPageCsv = async ()=>{
        await saveCsv('trips.csv',
            ...
            filtered.map(t=>[
                t.id, t.trip||'-', t.terminal_from, t.terminal_to, t.departure_time,
            ...
            <div className="rounded-2xl border bg-white p-4 overflow-x-auto">
                <table className="min-w-[1000px] w-full text-sm">
                    <thead><tr className="text-left text-indigo-900/60">
                        ...
                        <th className="py-2">Sefer</th><th>Kalkış</th><th>Varış</th><th>Tarih</th><th>Bilet</th><th>Gelir</th><th>Aktif</th><th className="text-right">CSV</th>
                    </tr></thead>
                    <tbody>
                    {filtered.map(t=>(
