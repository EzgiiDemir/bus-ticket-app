// app/dashboard/ik/toros/layout.tsx
'use client';
...
const NAV: NavItem[] = [
    { href: '/dashboard/ik/toros', label: 'Dashboard', Icon: LayoutDashboard },
    { href: '/dashboard/ik/toros/approvals', label: 'Onaylar', Icon: CheckCircle2 },
    { href: '/dashboard/ik/toros/Company', label: 'Firmam', Icon: Building2 },
    { href: '/dashboard/ik/toros/trips', label: 'Seferler', Icon: Bus },
    { href: '/dashboard/ik/toros/customers', label: 'Müşteriler', Icon: Users },
    { href: '/dashboard/ik/toros/personnel', label: 'Personeller', Icon: Users },
    { href: '/dashboard/ik/toros/account', label: 'Hesabım', Icon: User },
];

function Guard({ children }: { children: React.ReactNode }) {
    const { isLoading, token, user } = myAppHook() as any;
    const isToros =
        (user?.company?.code || '').toLowerCase() === 'toros' ||
        /(^|\s)toros(\s|$)/i.test(user?.company?.name || '');
    ...
    if (!isToros) return <div className="p-6">Bu bölüm Toros İK içindir.</div>;
    return <>{children}</>;
}
            ...
            <div className="min-h-screen bg-gray-50">
                <header className="lg:hidden sticky top-0 z-40 flex items-center justify-between px-4 py-3 border-b bg-white">
                    <div className="flex items-center gap-2">
                        ...
                        <div className="text-sm font-semibold text-indigo-900">Toros Paneli</div>
                    </div>
                    <button aria-label="Menüyü aç/kapat" aria-expanded={open} aria-controls="mobile-sidebar"
                            onClick={() => setOpen(o => !o)} className="p-2 rounded-lg text-indigo-900">
