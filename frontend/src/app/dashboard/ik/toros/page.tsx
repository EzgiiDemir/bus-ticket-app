// app/dashboard/ik/mavi/page.tsx
'use client';
...
export default function Page(){
    ...
    // Yaklaşan seferler CSV
    const exportUpcomingPage = async ()=>{
        await saveCsv('yaklasan_seferler.csv',
            ['ID','Sefer','Kalkış','Varış','Tarih','Satış','Gelir','Aktif'],
            upcoming.map(t=>[
                t.id, t.trip||'-', t.terminal_from||'-', t.terminal_to||'-', fmt(t.departure_time),
                Number(t.orders||0), String(t.revenue ?? 0), Number(t.is_active)?'Evet':'Hayır'
            ])
        );
    };
    const exportUpcomingOne = async (t:Trip)=>{
        await saveCsv(`sefer_${t.id}.csv`,
            ['ID','Sefer','Kalkış','Varış','Tarih','Satış','Gelir','Aktif'],
            [[ t.id, t.trip||'-', t.terminal_from||'-', t.terminal_to||'-', fmt(t.departure_time), Number(t.orders||0), String(t.revenue ?? 0), Number(t.is_active)?'Evet':'Hayır' ]]
        );
    };
    // En iyi müşteriler CSV
    const exportTopCustPage = async ()=>{
        await saveCsv('en_iyi_musteriler.csv',
            ['ID','Ad','E-posta','Sipariş','Toplam'],
            custs.slice(0,8).map(c=>[ c.id, c.name||'-', c.email||'-', Number(c.orders||0), String(c.total ?? 0) ])
        );
    };
    const exportCustOne = async (c:Cust)=>{
        await saveCsv(`musteri_${c.id}.csv`,
            ['ID','Ad','E-posta','Sipariş','Toplam'],
            [[ c.id, c.name||'-', c.email||'-', Number(c.orders||0), String(c.total ?? 0) ]]
        );
    };
    // Bekleyen onaylar CSV
    const exportPendingPage = async ()=>{
        await saveCsv('bekleyen_onaylar.csv',
            ['ID','Ad','E-posta','Firma Onayı','Admin Onayı','Başvuru'],
            pendingApprs.map(r=>[
                r.user?.id ?? '-', r.user?.name||'-', r.user?.email||'-', r.company_status, r.admin_status, r.created_at
            ])
        );
    };
    const exportPendingOne = async (r:Approval)=>{
        await saveCsv(`onay_${r.id}.csv`,
            ['ID','Ad','E-posta','Firma Onayı','Admin Onayı','Başvuru'],
            [[ r.user?.id ?? '-', r.user?.name||'-', r.user?.email||'-', r.company_status, r.admin_status, r.created_at ]]
        );
    };
    ...
    return (
        <div className="space-y-6 text-indigo-900">
            <div className="flex items-center justify-between">
                <h1 className="text-2xl font-bold">Toros • Genel Bakış</h1>
                <button
                    className="rounded-xl border px-3 py-2"
                    onClick={()=>{ (async()=>{ try{ const r=await api.get('/company/stats',{token}); setStats(await api.json(r)); }catch{} })(); }}
                >Yenile</button>
            </div>
            ...
            <div className="grid gap-4 lg:grid-cols-2">
                ...
                <section className="rounded-2xl border bg-white p-4">
                    <div className="mb-2 flex items-center justify-between">
                        ...
                        <button className="rounded-xl border px-3 py-1.5" onClick={exportUpcomingPage}>Sayfa CSV</button>
                    </div>
                            ...
                            {upcoming.map(t=>(
                                <tr key={t.id} className="border-t">
                                    ...
                                    <td className="text-right">
                                        <button className="rounded-lg border px-2 py-1" onClick={()=>exportUpcomingOne(t)}>CSV</button>
                                    </td>
                                </tr>
                            ))}
                            {!upcoming.length && <tr><td colSpan={9} className="py-6 text-center text-indigo-900/50">Kayıt yok</td></tr>}
                            </tbody>
                        </table>
                    </div>
                </section>
                ...
                <section className="rounded-2xl border bg-white p-4">
                    <div className="mb-2 flex items-center justify-between">
                        ...
                        <button className="rounded-xl border px-3 py-1.5" onClick={exportTopCustPage}>Sayfa CSV</button>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="min-w-[800px] w-full text-sm">
                            ...
                            {custs.slice(0,8).map(c=>(
                                <tr key={c.id} className="border-t">
                                    ...
                                    <td className="text-right">
                                        <button className="rounded-lg border px-2 py-1" onClick={()=>exportCustOne(c)}>CSV</button>
                                    </td>
                                </tr>
                            ))}
                            {!custs.length && <tr><td colSpan={6} className="py-6 text-center text-indigo-900/50">Kayıt yok</td></tr>}
                            </tbody>
                        </table>
                    </div>
                </section>
                ...
                <section className="rounded-2xl border bg-white p-4 lg:col-span-2">
                    <div className="mb-2 flex items-center justify-between">
                        ...
                        <button className="rounded-xl border px-3 py-1.5" onClick={exportPendingPage}>Sayfa CSV</button>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="min-w-[900px] w-full text-sm">
                            <thead>
                            ...
                            {pendingApprs.map(r=>(
                                <tr key={r.id} className="border-t">
                                    ...
                                    <td className="text-right">
                                        <button className="rounded-lg border px-2 py-1" onClick={()=>exportPendingOne(r)}>CSV</button>
                                    </td>
                                </tr>
                            ))}
                            {!pendingApprs.length && <tr><td colSpan={7} className="py-6 text-center text-indigo-900/50">Bekleyen onay yok</td></tr>}
                            </tbody>
                        </table>
                    </div>
                </section>
